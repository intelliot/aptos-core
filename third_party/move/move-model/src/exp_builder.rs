...
impl<'a> ExpBuilder<'a> {
    ...
    pub fn match_if(&self, exp: Exp) -> Option<(Exp, Exp)> {
        match exp.as_ref() {
            ExpData::IfElse(_, cond, if_true, if_false) if self.is_empty_exp(if_false) => {
    ...
    pub fn match_if_loop_cont(&self, exp: Exp, nest: usize, is_continue: bool) -> Option<Exp> {
        ...
        if self.is_loop_cont(&if_true, nest, is_continue) {
            Some(cond)
        } else {
            None
        }
    ...
    pub fn is_loop_cont(&self, exp: &Exp, nest: usize, is_continue: bool) -> bool {
        matches!(exp.as_ref(),
            ExpData::LoopCont(_, nest1, is_continue1) if *nest1 == nest && *is_continue1 == is_continue)
    }

    pub fn is_empty_exp(&self, exp: &Exp) -> bool {
        matches!(exp.as_ref(), ExpData::Sequence(_, stms) if stms.is_empty())
    }
