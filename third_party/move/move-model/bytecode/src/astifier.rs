...
impl<'a> IfElseTransformer<'a> {
    ...
    fn try_make_if_else(&self, exp: Exp) -> Option<Exp> {
        ...
        if self.builder.is_loop_cont(&outer_break, 0, false) {
        ...
        if self.builder.is_loop_cont(&inner_last, 1, false) && loop_binding_count == 1 {
            // found if-then-else
    ...
    fn try_make_if(&self, exp: Exp) -> Option<Exp> {
        ...
        if !self.builder.is_loop_cont(&last, 0, false) {
            return None;
        }
