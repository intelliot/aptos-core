...
pub struct VersionedGroupData<K, T, V> {
    ...
    group_sizes: DashMap<K, GroupSizes>,

    // Stored separately from group sizes to avoid R-W lock contention between
    // latest group contents preparation and the size updates.
    ...
    > VersionedGroupData<K, T, V>
{
    ...
    pub fn write(
        ...
        if !(group_sizes.size_has_changed && changes_behavior) {
            let (size_changed, update_flag) = group_sizes
                ...
                .map(|(idx, prev_size)| {
                    (
                        prev_size.cell != size,
                        // Update the size_has_changed flag is the entry isn't the base value
