...
static COMMIT_TIME: AtomicUsize = AtomicUsize::new(0);
static FINALIZE_TIME: AtomicUsize = AtomicUsize::new(0);
static LAST_COMMITTED_TIME: AtomicUsize = AtomicUsize::new(0);
static VM_TIME: AtomicUsize = AtomicUsize::new(0);
    ...
    X: Executable + 'static,
{
    ...
    fn execute(
        ...
        let vm_timer = Instant::now();
        let execute_result = executor.execute_transaction(&sync_view, txn, idx_to_execute);
        VM_TIME.fetch_add(vm_timer.elapsed().as_nanos() as usize, Ordering::Relaxed);
    ...
    fn worker_loop(
        ...
        loop {
            while scheduler.should_coordinate_commits() {
                ...
                COMMIT_TIME.fetch_add(
                    commit_timer.elapsed().as_nanos() as usize,
                    Ordering::Relaxed,
                );
    ...
    pub(crate) fn execute_transactions_parallel(
        ...
        VM_TIME.store(0, Ordering::Relaxed);
        FINALIZE_TIME.store(0, Ordering::Relaxed);
        LAST_COMMITTED_TIME.store(0, Ordering::Relaxed);
        COMMIT_TIME.store(0, Ordering::Relaxed);
        ...
        // println!(
        //     "VM total TIME micros = {:?}, commit time micros = {:?} commit fin {}, out of which fin {} last committed {}",
        //     VM_TIME.load(Ordering::Relaxed) / 1000,
        //     COMMIT_TIME.load(Ordering::Relaxed) / 1000,
        //     FINALIZE_TIME.load(Ordering::Relaxed) / 1000,
        //     LAST_COMMITTED_TIME.load(Ordering::Relaxed) / 1000,
        // );
